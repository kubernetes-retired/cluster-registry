...
func testClusterCreate(t *testing.T, clientset *crclientset.Clientset, clusterName string) {
	cluster, err := clientset.ClusterregistryV1alpha1().Clusters(testNamespace).Create(&v1alpha1.Cluster{
...
func testClusterGet(t *testing.T, clientset *crclientset.Clientset, clusterName string) {
	cluster, err := clientset.ClusterregistryV1alpha1().Clusters(testNamespace).Get(clusterName,
		metav1.GetOptions{})
...
func testClusterUpdate(t *testing.T, clientset *crclientset.Clientset, clusterName string) {
	cluster, err := clientset.ClusterregistryV1alpha1().Clusters(testNamespace).Get(clusterName, metav1.GetOptions{})
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	cluster, err = clientset.ClusterregistryV1alpha1().Clusters(testNamespace).Update(cluster)
...
func testClusterDelete(t *testing.T, clientset *crclientset.Clientset, clusterName string) {
	err := clientset.ClusterregistryV1alpha1().Clusters(testNamespace).Delete(clusterName,
	...
	_, err = clientset.ClusterregistryV1alpha1().Clusters(testNamespace).Get(clusterName, metav1.GetOptions{})
